...
class TransTabFeatureExtractor:
    ...
    def __init__(self,
        ...
        if not self.ignore_duplicate_cols:
            for col in duplicate_cols:
                logger.error(
                    f'Find duplicate cols named `{col}`, please process the raw data or set `ignore_duplicate_cols` to True!')
    ...
    def __call__(self, x, shuffle=False) -> Dict:
        ...
        if len(num_cols) > 0:
            ...
            # 这里可以看出以其他部分的不同，这里只将列名进行了tokenizer操作
    ...
    def _check_column_overlap(self, cat_cols=None, num_cols=None, bin_cols=None):
        ...
        if org_length == 0:
            logger.warning(
                'No cat/num/bin cols specified, will take ALL columns as categorical! Ignore this warning if you specify the `checkpoint` to load the model.')
            return True, []
    ...
    def forward(self,
        ...
        if x_num is not None and num_col_input_ids is not None:
            num_col_emb = self.word_embedding(
                num_col_input_ids.to(self.device))  # number of cat col, num of tokens, embdding size
...
class TransTabLinearClassifier(nn.Module):
    def __init__(self,
        ...
        # LayerNorm: 对每一个batch size进行归一化处理
        self.norm = nn.LayerNorm(hidden_dim)
...
class TransTabLinearRegression(nn.Module):
    def __init__(self,
                 hidden_dim=128) -> None:
        super().__init__()
        self.fc = nn.Linear(hidden_dim, 1)
...
class TransTabModel(nn.Module):
    ...
    def update(self, config):
        ...
        config: dict
            ...
            key num_class:int is to specify the number of classes for finetuning on a new WorldHappiness_Corruption_2015_2020.
...
class TransTabRegression(TransTabModel):
    ...
    def __init__(self,
        ...
        self.clf = TransTabLinearRegression(hidden_dim=hidden_dim)
...
class TransTabForCL(TransTabModel):
    ...
    def self_supervised_contrastive_loss(self, features):
        ...
        logits_mask = torch.scatter(torch.ones_like(mask), 1,
                                    torch.arange(batch_size * anchor_count).view(-1, 1).to(features.device), 0)
