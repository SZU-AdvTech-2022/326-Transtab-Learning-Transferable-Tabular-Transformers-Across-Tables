...
# 应该考虑一个问题，多个数据集到底会发生什么，最终的结果是什么？
    ...
    if isinstance(dataname, list):
        ...
        for dataname_ in dataname:
            allset, trainset, valset, testset, cat_cols, num_cols, bin_cols = \
                load_single_data(dataname_, dataset_config=dataset_config, encode_cat=encode_cat, data_cut=data_cut,
                                 seed=seed)
            # extend:  一次性再列表中扩充多个值
...
def load_single_data(dataname, dataset_config=None, encode_cat=False, data_cut=None, seed=123):
    '''Load tabular WorldHappiness_Corruption_2015_2020 from local or from openml public database.
    args:
    ...
    if os.path.exists(dataname):
        ...
        if os.path.exists(ftfile):
            with open(ftfile, 'r') as f:
                # strip: 删除开头是指定字符的字符，直到开头和结尾不是为止
        ...
        # 这里有点疑问，难道不使用'credit-g'就不会执行这一段代码了吗？
        if dataname in dataset_config:
            if 'bin' in dataset_config[dataname]: bin_cols = [c for c in cat_cols if
                                                              c in dataset_config[dataname]['bin']]
        else:
            bin_cols = []
        # 这一段代码应该是前面的代码可能对bin类型数据有误判，所以要删除
        cat_cols = [c for c in cat_cols if c not in bin_cols]

        # encode target label
        # 排列之后在进行表转化，比如[1, 3, 3, 7, 3, 1]经过处理之后就变成了[0, 1, 1, 2, 1, 0]
    ...
    # process num
    if len(num_cols) > 0:
        # fillna: 使用指定的方法填充NA和NAN的值, 使用众数进行填充，[0]是因为因为众数可能不止一个
    ...
    if len(bin_cols) > 0:
        ...
        if dataname in dataset_config:
            # 把所有是true的单元格值替换为1，否则是0
            if 'binary_indicator' in dataset_config[dataname]:
                X[bin_cols] = X[bin_cols].astype(str).applymap(
                    lambda x: 1 if x.lower() in dataset_config[dataname]['binary_indicator'] else 0).values
            else:
                X[bin_cols] = X[bin_cols].astype(str).applymap(
